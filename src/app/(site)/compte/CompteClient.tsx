          ...
          <nav className="px-3 py-4">
            ...
            {NAV.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm mb-1 transition-colors ${tab === item.id
          ...
          {tab === 'commandes' && (
            <>
              <div className="flex items-center justify-between mb-8">
                <h1 className="text-2xl font-serif text-neutral-900">Mes commandes</h1>
                <span className="text-xs text-neutral-400">{orders.length} commande{orders.length > 1 ? 's' : ''}</span>
              </div>
              ...
              {orders.length === 0 ? (
                <div className="text-center py-24 bg-white rounded-2xl border border-neutral-100">
                  ...
                  <p className="text-neutral-500 text-sm mb-6">Vous n&apos;avez pas encore passé de commande.</p>
                  <Link
                    href="/boutique"
                    className="inline-block bg-neutral-900 text-white px-8 py-3 text-xs uppercase tracking-widest hover:bg-neutral-800 transition-colors rounded-lg"
                  >
                ...
                <div className="space-y-4">
                  {orders.map(order => {
                    ...
                    const date = order.createdAt
                      ...
                      : '';
                    return (
                      <div key={order._id} className="bg-white rounded-2xl border border-neutral-100 overflow-hidden hover:border-neutral-200 transition-colors">

                        {/* En-tête commande */}
                        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-neutral-50 bg-neutral-50/50">
                          <div className="flex items-center gap-4">
                            <div>
                              <p className="text-[10px] uppercase tracking-widest text-neutral-400 mb-0.5">Commande</p>
                              <p className="text-sm font-mono font-semibold text-neutral-800">#{formatOrderNumber(order.orderNumber)}</p>
                            </div>
                            {date && (
                              <div className="h-8 w-px bg-neutral-200 hidden sm:block" />
                            )}
                            {date && (
                              <p className="text-xs text-neutral-400 hidden sm:block">{date}</p>
                            )}
                          </div>
                          <div className="flex items-center gap-3">
                            <span className={`text-[10px] font-semibold uppercase tracking-wider px-3 py-1.5 rounded-full border ${s.color}`}>
                              {s.label}
                            </span>
                            {order.status !== 'cancelled' && (
                              <a
                                ...
                                className="flex items-center gap-1.5 text-xs text-neutral-500 bg-white border border-neutral-200 px-3 py-1.5 rounded-lg hover:bg-neutral-900 hover:text-white hover:border-neutral-900 transition-all"
                              >
                        ...
                        <div className="px-6 py-4 space-y-3">
                          {order.items.map((item, i) => (
                            <div key={i} className="flex items-center gap-4">
                              {item.image ? (
                                <div className="w-12 h-12 flex-shrink-0 overflow-hidden rounded-xl bg-neutral-50">
                                  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                </div>
                              ) : (
                                <div className="w-12 h-12 flex-shrink-0 rounded-xl bg-neutral-100 flex items-center justify-center">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-neutral-400 mt-0.5">Qté : {item.qty}</p>
                              </div>
                        ...
                        <div className="px-6 py-3 border-t border-neutral-100 flex justify-between items-center">
                          <p className="text-xs text-neutral-400 sm:hidden">{date}</p>
                          <p className="text-xs text-neutral-400 hidden sm:block">Total commande</p>
