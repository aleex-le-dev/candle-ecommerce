...
export default function AdminPage() {
  ...
  const [counts, setCounts] = useState<{ products: number; promos: number; histoire: number; valeurs: number; categories: number } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/categories').then(r => r.json()).then(d => { if (Array.isArray(d)) setDbCategories(d.map((c: any) => c.name)); });
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        >
                          {(dbCategories.length ? dbCategories : CATEGORIES).map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </div>
